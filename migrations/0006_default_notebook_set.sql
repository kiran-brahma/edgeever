...
SELECT 'nb_projects', NULL, 'Work Projects', 'work-projects', 'notebook', '#2563eb', 20
WHERE NOT EXISTS (
  SELECT 1 FROM notebooks WHERE id = 'nb_projects' OR slug = 'work-projects'
);

INSERT INTO notebooks (id, parent_id, name, slug, icon, color, sort_order)
SELECT 'nb_learning', NULL, 'Learning Resources', 'learning-resources', 'notebook', '#7c3aed', 30
WHERE NOT EXISTS (
  SELECT 1 FROM notebooks WHERE id = 'nb_learning' OR slug = 'learning-resources'
);

INSERT INTO notebooks (id, parent_id, name, slug, icon, color, sort_order)
SELECT 'nb_creative', NULL, 'Creative Ideas', 'creative-ideas', 'notebook', '#db2777', 40
WHERE NOT EXISTS (
  SELECT 1 FROM notebooks WHERE id = 'nb_creative' OR slug = 'creative-ideas'
);

INSERT INTO notebooks (id, parent_id, name, slug, icon, color, sort_order)
SELECT 'nb_personal', NULL, 'Personal Life', 'personal-life', 'notebook', '#ea580c', 50
WHERE NOT EXISTS (
